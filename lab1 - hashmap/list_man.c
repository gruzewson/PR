...
ListManager* createManager(void* (*f_hash)(void*), void (*f_print)(ListManager*), int (*f_comp)(void*, void*))
{
    ListManager* manager = (ListManager*)malloc(sizeof(ListManager));
    if (manager == NULL) {
        printf("Memory allocation failed.\n");
        exit(1);
    }
    manager->hashList = NULL;
    manager->f_hash = f_hash;
    manager->f_print = f_print;
    manager->f_comp = f_comp;
    return manager;
}
